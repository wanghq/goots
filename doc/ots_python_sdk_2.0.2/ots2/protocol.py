...
class OTSProtocol:
    ...
    def _check_headers(self, headers, body):
    ...
    def _check_authorization(self, query, headers):
    ...
    def make_request(self, api_name, *args, **kwargs):
        ...
        if self.logger.level <= logging.DEBUG:
            ...
            self.logger.debug("OTS request, API: %s, Protobuf: %s" % (
                api_name, 
                text_format.MessageToString(
                    proto, as_utf8=True, as_one_line=True
                )
            ))
    ...
    def parse_response(self, api_name, status, headers, body):
        ...
        if self.logger.level <= logging.DEBUG:
            ...
            request_id = self._get_request_id_string(headers)
            self.logger.debug("OTS response, API: %s, Protobuf: %s, RequestID: %s." % (
                api_name, text_format.MessageToString(proto, as_utf8=True, as_one_line=True), 
                request_id
            ))
        return ret
    ...
    def handle_error(self, api_name, query, status, reason, headers, body):
        ...
        try:
            self._check_headers(headers, body)
            if status != 403:
                self._check_authorization(query, headers)
